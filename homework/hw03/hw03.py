...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t):
        ...
        new_branches = [replace_leaf(i,find_value, replace_value) for i in branches(t)]
...
def preorder(t):
    ...
    while stack:
        temp = stack.pop()
        res.append(label(temp))
        stack.extend(reversed(branches(temp)))
    return res
...
def has_path(t, word):
    ...
    while queue:
        i, depth = queue.pop(0)
        if label(i) == word[depth]:
            if len_word == depth + 1:
                return True
            queue.extend((j ,depth + 1) for j in branches(i))
    return False
...
def quadratic(x, a, b, c):
    ...
    low = a * lower_bound(x) ** 2 + b * lower_bound(x)+c
    if lower_bound(x) < -b/(2*a) and upper_bound(x) > -b/(2*a):
        mid = c - b ** 2 / (4 * a)
        return interval(min(mid, upper, low), max(mid, upper, low))
    else:
        return interval(min(upper, low), max(upper, low))
