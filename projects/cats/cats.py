...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def check(paragraph):
        list_para = split(lower(remove_punctuation(paragraph)))
        for m in topic:
            if m in list_para:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if not typed_words:
        return 0.0
    for m in typed_words:
        if total < length and m == reference_words[total]:
            count += 1
        total += 1
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    result = min(valid_words, key = lambda item: diff_function(user_word, item, limit))
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9 
    time_all = []
    for m in times_per_player:
        time_all.append([m[i] - m [i - 1] for i in range(1, len(m))])
    ...
    for word in word_indices:
        ...
        for player in player_indices:
            if time(game, player, word) < min_time:
                ...
                min_index = player
        fastest[min_index].append(word_at(game, word))
    return fastest
    # END PROBLEM 10
