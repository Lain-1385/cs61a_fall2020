...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        ...
        for i in leaves:
            lst_leaves += [tree(i)]
        return tree(label(t), lst_leaves)
    else:
        lst_branches = []
        for i in branches(t):
            lst_branches += [sprout_leaves(i, leaves)]
        return tree(label(t), lst_branches)
...
def add_trees(t1, t2):
    ...
    len_t2 = len(br_t2)
    lst = []
    for i in range(max(len_t1, len_t2)):
        if(i < len_t1 and i < len_t2):
            lst += [add_trees(br_t1[i], br_t2[i])]
        elif i >= len_t1:
            lst += [br_t2[i]]
        else:
            lst += [br_t1[i]]
